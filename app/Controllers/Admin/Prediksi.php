...
class Prediksi extends BaseController
{
    ...
    {   
        ...
        //mencari mape
        $pembatas2 = ($tahun-1)."-".($bulan-1)."-01";
        $pembatas1 = $tahun."-".$bulan."-01";
        $hasil = array();
        $hasiler[0] = ['wma' => 'wma3', 'error' => $this->hasilModel->select('sum(error3)/9 as error')->where("bulan BETWEEN '".$pembatas2."' AND '".$pembatas1."'")->orderBy('bulan', 'DESC')->limit(9)->findAll()[0]['error'], 'nilai' => $this->hasilModel->select('wma3')->where('bulan', $tanggal1)->findAll()[0]['wma3']];
        $hasiler[1] = ['wma' => 'wma4', 'error' => $this->hasilModel->select('sum(error4)/8 as error')->where("bulan BETWEEN '".$pembatas2."' AND '".$pembatas1."'")->orderBy('bulan', 'DESC')->limit(8)->findAll()[0]['error'], 'nilai' => $this->hasilModel->select('wma4')->where('bulan', $tanggal1)->findAll()[0]['wma4']];
        $hasiler[2] = ['wma' => 'wma5', 'error' => $this->hasilModel->select('sum(error5)/7 as error')->where("bulan BETWEEN '".$pembatas2."' AND '".$pembatas1."'")->orderBy('bulan', 'DESC')->limit(7)->findAll()[0]['error'], 'nilai' => $this->hasilModel->select('wma5')->where('bulan', $tanggal1)->findAll()[0]['wma5']];
        $hasiler[3] = ['wma' => 'wma6', 'error' => $this->hasilModel->select('sum(error6)/6 as error')->where("bulan BETWEEN '".$pembatas2."' AND '".$pembatas1."'")->orderBy('bulan', 'DESC')->limit(6)->findAll()[0]['error'], 'nilai' => $this->hasilModel->select('wma6')->where('bulan', $tanggal1)->findAll()[0]['wma6']];
